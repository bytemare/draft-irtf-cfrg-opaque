...
def test_3dh_new():
    ...
    # Protocol messages
    vector["RegistrationRequest"] = to_hex(reg_request.serialize())
    vector["RegistrationRequestBlind"] = to_hex(config.oprf_suite.group.serialize_scalar(metadata))
    vector["RegistrationResponse"] = to_hex(reg_response.serialize())
    vector["RegistrationUpload"] = to_hex(record.serialize())
    # vector["CredentialRequest"] = to_hex(serialized_cred_request)
    # vector["CredentialRequestBlind"] = to_hex(config.oprf_suite.group.serialize_scalar(cred_metadata))
    # vector["CredentialResponse"] = to_hex(serialized_cred_response)
    vector["KE1"] = to_hex(ke1)
    vector["KE2"] = to_hex(ke2)
    vector["KE3"] = to_hex(ke3)

    vector["info1"] = to_hex(info1)
    vector["info2"] = to_hex(info2)
    vector["client_nonce"] = to_hex(client_kex.nonceU)
    vector["server_nonce"] = to_hex(server_kex.nonceS)
    vector["eskU"] = to_hex(I2OSP_le(client_kex.eskU, 32))
    vector["epkU"] = to_hex(ristretto255.ENCODE(*client_kex.epkU))
    vector["eskS"] = to_hex(I2OSP_le(server_kex.eskS, 32))
    vector["epkS"] = to_hex(ristretto255.ENCODE(*server_kex.epkS))
    vector["shared_secret"] = to_hex(server_session_key)

    # Intermediate computations
    vector["kU"] = to_hex(config.oprf_suite.group.serialize_scalar(kU))
    vector["envU_nonce"] = to_hex(core.envelope_nonce)
    vector["envU"] = to_hex(record.envU.serialize())
